...
- **First light is the floor.** No channel — sensor, relay, rumor, or market — delivers information about an event at A to HQ before `|A−HQ|/c`. The triangle inequality forbids it (§3, Precursors & rumor). Any design that appears to beat it is either leaking ground truth or is really about *upstream conditions*, which is a different and legitimate mechanism.
...
**Why this works here and wouldn't in Aurora: light-lag creates horizons for free.** Instant information means anything can change anything at any time, so nothing has a horizon. A captain 40 light-minutes out is *provably unable* to learn anything for 40 minutes. **The mechanic that makes this game expensive is the same mechanic that makes it cheap.**
...
**What survives everything:** the root-find when a ship is genuinely at a decision point with another moving endpoint. Cost scales with **decision points × in-scope events**, not with ships × ticks. Light-lag is what's expensive here, not orbits — cutting orbital fidelity buys back nothing; only horizons do.
...
The genuinely pre-event channel is different, and it doesn't need light from A at all: **rumors are inferences from causally upstream facts that already arrived.**
